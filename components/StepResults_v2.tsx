...
const StepResults: React.FC<Props> = ({ results, userType, onBack, onReset, onShowStats }) => {
  ...
  return (
    <div className="flex flex-col min-h-full">
      <div id="results-content" className="animate-fade-in flex-grow p-4 space-y-8">
        {/* --- PAGE 1: RESUMEN Y AHORRO --- */}
        <div className="print:break-after-page space-y-8">
          ...
          <div className="bg-gradient-to-r from-violet-600 to-fuchsia-600 rounded-2xl p-8 text-white shadow-xl flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
          ...
          <div className="space-y-4">
            {isGD && !results.details?.["Tasa Mun. Rosario (0.6% + 1.8%)"] && (
              <div className="bg-blue-50 border-l-4 border-blue-400 p-4 rounded-r shadow-sm">
                <div className="flex items-start">
                  ...
                  <div className="ml-3"><p className="text-sm text-blue-700 font-medium">Si el suministro se encuentra en la ciudad de Rosario, el cálculo de los impuestos puede presentar una leve variación de aproximadamente 2%.</p></div>
                </div>
              </div>
            )}
            {userType === UserType.EPE_NO_PROSUMIDOR_GD && (
              <div className="bg-blue-50 border-l-4 border-blue-400 p-4 rounded-r shadow-sm">
                <div className="flex items-start">
                  <div className="flex-shrink-0"><Info className="h-5 w-5 text-blue-400" /></div>
                  <div className="ml-3"><p className="text-sm text-blue-700 font-medium">Los reconocimientos resultan aplicables únicamente en caso de contar con Certificado MiPyME vigente.</p></div>
                </div>
              </div>
            )}
            {userType === UserType.EPE_NO_PROSUMIDOR_RESIDENCIAL && (
              <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-r shadow-sm">
                <div className="flex items-start">
                  ...
                  <div className="ml-3"><p className="text-sm text-yellow-700 font-medium">Si el usuario no corresponde a la categoría SS, los ahorros estimados podrían encontrarse sobrevalorados.</p></div>
                </div>
              </div>
            )}
          </div>

          {/* Technical Info (Only for non-prosumers as in current logic, but following structure request) */}
          {!isProsumidor && (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-6 rounded-2xl shadow-md border-t-4 bg-orange-50 border-orange-500 flex flex-col justify-center text-center">
                  <h3 className="text-sm font-bold text-gray-900 mb-1 uppercase tracking-wider">Período estimado de recupero de la inversión</h3>
                  <p className="text-3xl font-extrabold text-gray-900">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-md border border-gray-100 flex flex-col justify-center text-center">
                  <h3 className="text-sm font-semibold text-gray-500 mb-2 uppercase tracking-wider">Vida útil de los paneles solares</h3>
                  <p className="text-sm text-gray-600 font-medium leading-relaxed">El sistema de paneles solares está diseñado para ofrecer un rendimiento óptimo durante una vida útil estimada de 25 años.</p>
                </div>
              </div>
            </>
          )}
        ...
        {/* --- PAGE 2: COMPOSICION Y EFICIENCIA --- */}
        <div className="print:break-after-page space-y-8">
          ...
          <div className="bg-gradient-to-br from-teal-800 to-emerald-900 text-white rounded-xl p-8 shadow-xl">
        ...
        {/* --- PAGE 3: FEEDBACK Y RECOMENDACIONES --- */}
        <div className="space-y-8 pb-12">
          <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            {saved ? (
              <div className="p-12 text-center space-y-4 animate-fade-in">
                <div className="inline-flex items-center justify-center w-20 h-20 bg-emerald-100 text-emerald-600 rounded-full mb-4"><Check size={40} strokeWidth={3} /></div>
              ...
              <>
                <div className="bg-violet-50 px-6 py-4 border-b border-violet-100"><h3 className="font-bold text-violet-800 flex items-center gap-2">¿Obtuvo la respuesta que buscaba?</h3></div>
                <div className="p-8 space-y-6">
                  <div className="flex justify-center gap-8">
                    <button onClick={() => setResolved(true)} className={`flex flex-col items-center gap-2 p-6 rounded-2xl border-2 transition-all transform hover:scale-105 ${resolved === true ? 'bg-emerald-50 border-emerald-500 shadow-md scale-105' : 'bg-white border-gray-100 hover:border-emerald-200'}`}><div className={`p-4 rounded-full ${resolved === true ? 'bg-emerald-500 text-white' : 'bg-emerald-100 text-emerald-600'}`}><Check size={32} strokeWidth={3} /></div><span className={`font-black text-xl ${resolved === true ? 'text-emerald-700' : 'text-gray-400'}`}>SI</span></button>
                    <button onClick={() => setResolved(false)} className={`flex flex-col items-center gap-2 p-6 rounded-2xl border-2 transition-all transform hover:scale-105 ${resolved === false ? 'bg-red-50 border-red-500 shadow-md scale-105' : 'bg-white border-gray-100 hover:border-red-200'}`}><div className={`p-4 rounded-full ${resolved === false ? 'bg-red-500 text-white' : 'bg-red-100 text-red-600'}`}><X size={32} strokeWidth={3} /></div><span className={`font-black text-xl ${resolved === false ? 'text-red-700' : 'text-gray-400'}`}>NO</span></button>
                  </div>
                  <div className="space-y-2">
                    <label className="block text-sm font-bold text-gray-700 uppercase tracking-wider">Observaciones/recomendaciones</label>
                    <textarea value={observations} onChange={(e) => setObservations(e.target.value)} placeholder="Escribe aquí tus comentarios..." className="w-full p-4 border border-gray-200 rounded-xl focus:ring-2 focus:ring-violet-500 outline-none min-h-[100px] transition-all" />
                  </div>
                  <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex items-center gap-3 shadow-sm animate-fade-in"><Info className="text-blue-600 w-5 h-5 flex-shrink-0" /><p className="text-sm text-blue-800 font-bold">Dejá tu correo de contacto para solucionar tus inquietudes</p></div>
                  <div className="flex justify-center">
                    <button onClick={handleSaveFeedback} disabled={resolved === null || isSaving} className={`px-10 py-4 rounded-xl font-black text-white shadow-lg transition-all transform active:scale-95 ${resolved === null || isSaving ? 'bg-gray-300 cursor-not-allowed' : 'bg-violet-600 hover:bg-violet-700 hover:-translate-y-1 hover:shadow-xl'}`}>{isSaving ? 'Guardando...' : 'Guardar respuestas'}</button>
                  </div>
                </div>
              </>
            )}
