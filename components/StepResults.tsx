...
import { ArrowLeft, RefreshCw, Leaf, Trees, Banknote, AlertCircle, Printer, PieChart as PieIcon, List, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className="flex flex-col min-h-full">
      <div className="animate-fade-in space-y-8 flex-grow">
        <div className="flex justify-between items-center border-b border-gray-200 pb-4">
          ...
          <button onClick={() => window.print()} className="no-print flex items-center gap-2 px-4 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-900 transition-colors shadow-sm">
            <Printer size={18} /><span>Guardar PDF</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print-break-inside">
          <div className={`p-6 rounded-2xl shadow-md border-t-4 ${isProsumidor ? 'bg-violet-50 border-violet-500' : 'bg-orange-50 border-orange-500'}`}>
            ...
            <span className="inline-block px-3 py-1 bg-white text-xs font-bold rounded-full shadow-sm text-gray-600 border border-gray-100">Con Régimen Prosumidor</span>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-md border-t-4 border-gray-300">
            ...
            <span className="inline-block px-3 py-1 bg-gray-100 text-xs font-bold rounded-full text-gray-500">Sin Régimen</span>
          </div>
        </div>
        ...
        {isProsumidor && (
          <div className="print-break-inside bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-10">
              <div><div className="flex justify-between items-end mb-2"><p className="text-xs text-gray-500 font-bold uppercase tracking-wide">Energía Autoconsumida / Generada</p><span className="text-2xl font-extrabold text-violet-600">{results.autoconsumoPercent !== undefined ? formatNumber(results.autoconsumoPercent, 1) : 0}%</span></div><div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden"><div className="bg-violet-600 h-4 rounded-full" style={{ width: `${Math.min(100, Math.max(0, results.autoconsumoPercent || 0))}%` }}></div></div></div>
              {results.injectionPercent !== undefined && (<div><div className="flex justify-between items-end mb-2"><p className="text-xs text-gray-500 font-bold uppercase tracking-wide">Energía Inyectada / Generada</p><span className="text-2xl font-extrabold text-orange-500">{formatNumber(results.injectionPercent, 1)}%</span></div><div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden"><div className="bg-orange-500 h-4 rounded-full" style={{ width: `${Math.min(100, Math.max(0, results.injectionPercent || 0))}%` }}></div></div></div>)}
            </div>
          </div>
        )}
                ...
                <table className="min-w-full text-sm divide-y divide-gray-100">
                  <tbody className="bg-white divide-y divide-gray-100">
                    {Object.entries(results.details).map(([key, value]) => (
                      <tr key={key} className="hover:bg-gray-50"><td className="px-6 py-3 font-medium text-gray-700">{key}</td><td className="px-6 py-3 text-right font-mono text-gray-600">{typeof value === 'number' ? formatNumber(value, 2) : value}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
