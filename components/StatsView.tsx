...
import { ArrowLeft, RefreshCw, Check, X, Lock, Users, MessageSquare, LogIn, LogOut } from 'lucide-react';
import { UserType } from '../types';
import { db, auth } from '../firebase';
import { collection, getDocs, query, orderBy, getDocFromServer } from 'firebase/firestore';
...
const StatsView: React.FC<Props> = ({ onBack }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const feedbackData = querySnapshot.docs.map(doc => doc.data() as FeedbackEntry);
      setData(feedbackData);
    } catch (err: any) {
      if (err.message.includes('permission-denied')) {
        setError('No tienes permisos suficientes para ver las estadísticas. Asegúrate de ser administrador.');
      } else {
    ...
    return (
      <div className="animate-fade-in flex flex-col items-center justify-center py-20 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-gray-500 max-w-xs mx-auto">Debes iniciar sesión con una cuenta autorizada para ver las estadísticas.</p>
        </div>
        <button 
          onClick={handleLogin}
          className="flex items-center gap-3 px-8 py-4 bg-white border-2 border-gray-100 rounded-2xl font-black shadow-xl hover:border-slate-800 transition-all group"
        >
          <LogIn size={20} className="text-slate-400 group-hover:text-slate-800 transition-colors" />
          Iniciar Sesión con Google
        </button>
        <button onClick={onBack} className="text-sm font-bold text-gray-400 hover:text-gray-600 underline underline-offset-4">Volver al simulador</button>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <p className="text-sm text-gray-500 font-medium uppercase tracking-wider">Admin: {user.email}</p>
                  ...
                  {data.length === 0 ? (
                    ...
                    data.map((entry, index) => (
                      <tr key={index} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4 font-bold text-gray-400">#{data.length - index}</td>
