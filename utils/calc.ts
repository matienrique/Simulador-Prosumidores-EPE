...
export const calculateProsumidorGD = (data: ProsumidorGDData): CalculationResult => {
  ...
  const totalPagarSinPros = subtotalGeneralSinPros - reconGSF_GD; // Asumiendo que reconGSF podría estar o no, pero "Sin Pros" teóricamente no lo tiene. 
  // CORRECCIÓN LÓGICA: Sin Prosumidores NO hay reconocimiento GSF ni EPE por inyección.
  // El cálculo original restaba reconGSF_GD. Si es "Sin Prosumidores", el usuario no genera, por tanto no cobra GSF.
  // Ajustamos totalPagarSinPros eliminando el crédito GSF.
  const totalPagarSinProsReal = subtotalGeneralSinPros; 

  const taxesPaidUser = (subtotalConsumoEnergia * (ivaCorrespondiente + 0.06 + 0.015 + percepcionCorrespondiente)) + leyGD + (capGD * (1 + ivaCorrespondiente + percepcionCorrespondiente));
  ...
  const totalSavings = totalPagarSinProsReal - totalPagar;
  ...
  return {
    ...
    details: {
      ...
      "Subtotal Energía Con Pros ($)": subtotalConsumoEnergia,
      "Total Impuestos Con Pros ($)": taxesPaidUser,
      "Total a Pagar Con Pros ($)": totalPagar,
...
export const calculateNoProsumidor = (data: NoProsumidorData): CalculationResult => {
  if (data.category === NoProsumidorCategory.GRAN_DEMANDA) {
    ...
    const co2 = 0.041 * (energiaGeneradaResto + energiaGeneradaValle + energiaGeneradaPico);
  ...
  // Rosario Logic
  const Orden_Mun_1592_62 = (category === NoProsumidorCategory.RESIDENCIAL && data.isRosario) ? 0.006 : 0;
  const Orden_Mun_1618_62 = (category === NoProsumidorCategory.RESIDENCIAL && data.isRosario) ? 0.018 : 0;
  ...
  return {
    ...
    details: { 
      ...
      ...(data.isRosario && category === NoProsumidorCategory.RESIDENCIAL ? { "Tasa Mun. Rosario (0.6% + 1.8%)": "Aplicada" } : {})
    }
  };
};